...
int t, n, l, ctr, cp;
string s;
bigint in[101], prim[101];
char ans[101];
vector<pair<bigint,int> > vs;
...
int main()
{
    ...
    for(int z = 0; z<t; z++)
    {
        ...
        cp++;
        prim[cp] = gcd(in[cp],in[cp-1]);
        for(int i = cp+1; i<=l; i++)
        {
            prim[i] = in[i-1]/prim[i-1];
        }
        for(int i = cp-1; i>=0; i--)
        {
            prim[i] = in[i]/prim[i+1];
        }
        for(int i = 0; i<=l; i++)
        {
            vs.push_back(make_pair(prim[i],i));
        }
        sort(vs.begin(),vs.end());
        for(int i = 0; i<=l; i++)
        {
            ans[vs[i].second] = char('A'+ctr);
            if((i<=l)&&(vs[i].first!=vs[i+1].first))
            {
                ctr++;
            }
        }
