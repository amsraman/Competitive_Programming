...
struct PowerSeries {
    ...
    PowerSeries operator+(const PowerSeries & p) { const
    ...
    friend PowerSeries deriv(const PowerSeries & p) {
        assert(!p.poly.empty());
        PowerSeries res((int) p.size(), 0);
        for(int i = 0; i + 1 < (int) p.size(); i++) res.poly[i] = p.poly[i];
        if(res.size() > 1) res.poly.pop_back();
        return res;
    }
    friend PowerSeries integral(const PowerSeries & p) {
        PowerSeries res((int) p.size() + 1, 0);
        for(int i = 0; i < (int) p.size(); i++) {
            res.poly[i + 1] = p.poly[i];
        }
        return res;
    }
    friend PowerSeries inv(const PowerSeries & p, size_t n) {
    ...
    friend PowerSeries log(PowerSeries & p, size_t n) {};
