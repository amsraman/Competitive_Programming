...
struct PowerSeries {
    ...
    friend pair<PowerSeries, PowerSeries> divmod(const PowerSeries & a, const PowerSeries & b) {
    ...
    T eval(T x) {
        T res = 0;
        for(int i = (int) poly.size() - 1; i >= 0; i--) res = (B::mul(res, x) + poly[i]) % B::mod;
        return res;
    }
    vector<T> eval(vector<T> & x) {
        int m = x.size(), sz = (1 << (__lg(m) + 1));
        vector<PowerSeries> st(2 * sz, PowerSeries(1, 1));
        for(int i = 0; i < m; i++) st[i + sz] = PowerSeries(vector<T>{B::mod - x[i], 1});
        for(int i = sz - 1; i > 0; i--) {
            st[i] = st[2 * i] * st[2 * i + 1];
        }
        vector<T> res(m);
        auto dfs = [&](auto rec, PowerSeries cur, int lo, int hi, int ind) -> void {
            if(lo == hi) {
                if(lo < m) res[lo] = cur.val(0);
                return;
            }
            int mid = (lo + hi) / 2;
            rec(rec, cur % st[2 * ind], lo, mid, 2 * ind);
            rec(rec, cur % st[2 * ind + 1], mid + 1, hi, 2 * ind + 1);
        };
        dfs(dfs, *this, 0, sz - 1, 1);
        return res;
    }
};
