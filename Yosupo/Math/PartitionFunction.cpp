...
template <class B>
vector<typename B::T> partition(int n) {
    B res(n + 1);
    for(int i = 0; i * (3 * i - 1) / 2 <= n; i++) {
        res.poly[i * (3 * i - 1) / 2] = (i & 1 ? B::mod - 1 : 1);
    }
    for(int i = 0; i * (3 * i + 1) / 2 <= n; i++) {
        res.poly[i * (3 * i + 1) / 2] = (i & 1 ? B::mod - 1 : 1);
    }
    return inv(res, n + 1).poly;
}

int main() {
    ios_base::sync_with_stdio(false); cin.tie(0);
    int n; cin >> n;
    vector<int> res = partition<PowerSeries<NTT>>(n);
    for(auto val: res) {
        cout << val << " ";
    }
    cout << '\n';
