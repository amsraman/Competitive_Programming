...
template <class B>
vector<typename B::T> stirling1(int n) {
    B acc(vector<int>{0, 1}), res(1, 1);
    while(n) {
        if(n & 1) {
            res = res * taylor_shift(acc, B::mod - res.size() + 1);
        }
        acc = acc * taylor_shift(acc, B::mod - acc.size() + 1), n >>= 1;
    }
    return res.poly;
}
...
int main() {
    ...
    int n; cin >> n;
    vector<int> res = stirling1<PowerSeries<NTT>>(n);
    for(auto val: res) {
        cout << val << " ";
    }
    cout << '\n';
