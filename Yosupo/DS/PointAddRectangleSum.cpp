...
int main() {
    ...
    for(int i = 0; i < n; i++) {
        ...
        queries[i][0] = 0;
        points.push_back({queries[i][1], queries[i][2]});
    }
    for(int i = n, t; i < n + q; i++) {
        cin >> queries[i][0];
        if(queries[i][0]) {
            cin >> queries[i][1] >> queries[i][2] >> queries[i][3] >> queries[i][4];
            points.push_back({queries[i][1], queries[i][2]}), points.push_back({queries[i][1], queries[i][4]});
            points.push_back({queries[i][3], queries[i][2]}), points.push_back({queries[i][3], queries[i][4]});
        } else {
            cin >> queries[i][1] >> queries[i][2] >> queries[i][3];
            points.push_back({queries[i][1], queries[i][2]});
        }
    }
    FenwickTree2D<int, ll> ft(points);
    for(array<int, 5> qr: queries) {
        if(qr[0]) {
            cout << ft.qry(qr[1], qr[3], qr[2], qr[4]) << '\n';
        } else {
            ft.upd(qr[1], qr[2], qr[3]);
        }
    }
}