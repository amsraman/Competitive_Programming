void main_lorentz(const string & s, vector<array<int, 3>> & reps, int lo, int hi) { // Requires Z
    if(lo == hi) {
        return;
    }
    int mid = (lo + hi) / 2, len1 = mid - lo + 1, len2 = hi - mid;
    main_lorentz(s, reps, lo, mid); main_lorentz(s, reps, mid + 1, hi);
    string u = s.substr(lo, mid - lo + 1), v = s.substr(mid + 1, hi - mid);
    string ru(u.rbegin(), u.rend()), rv(v.rbegin(), v.rend());
    vector<int> z1 = z_function(ru), z2 = z_function(v + "#" + u), z3 = z_function(ru + "#" + rv), z4 = z_function(v);
...
vector<array<int, 3>> main_lorentz(const string & s) { // Requires Suffix Array
    int n = s.length();
